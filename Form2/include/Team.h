...
private:
    ...
    std::string stadium;
    int founded_year;//can't be changed but if wrong init founded_year?
...
public:
    // Constructor
    Team();
    Team(int id, const std::string& name, const std::string& abbre, const std::string& stadium, int founded_year,
    ...
    void show_players();
    const std::vector<const Match*>& get_match() const;
    void set_match(const std::vector<const Match*>& matches);
